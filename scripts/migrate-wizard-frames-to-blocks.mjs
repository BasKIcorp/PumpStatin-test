 ...
 * Usage: node scripts/migrate-wizard-frames-to-blocks.mjs [profileId...]
 */
...
const profileIds =
  process.argv.slice(2).length > 0
    ...
    : fs.readdirSync(profilesDir).filter((d) =>
...
for (const profileId of profileIds) {
  ...
  console.log(
    changed
      ? `${profileId}: migrated → ${blockCount} blocks in wizard page`
