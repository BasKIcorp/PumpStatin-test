...
/** Cabinet UI without AppShell — for grid blocks */
export function CabinetPageContent() {
  ...
  const attach = async () => {
    if (!selectedProject || selectedIds.length === 0) {
      setError("Выберите проект и подборы");
      return;
    }
    setBusy(true);
    try {
      ...
      setError(e instanceof Error ? e.message : "Не удалось добавить подборы");
    } finally {
      setBusy(false);
    }
  };
  ...
  const buildProjectPdf = async (docType: "tkp" | "techsheet") => {
    ...
    try {
      ...
      downloadBlob(blob, `project-${selectedProject}-${docType}.pdf`);
    } catch (e) {
    ...
    <div className="p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href={wizardRoute} className="rounded border px-3 py-1.5 text-sm hover:bg-neutral-50">
            К подбору
          </Link>
          <button
            type="button"
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-lg border bg-white p-4">
            ...
            <ul className="space-y-2">
              {projects.map((p) => (
                <li key={p.id} className="rounded border px-2 py-2 text-sm">
          ...
          <section className="rounded-lg border bg-white p-4">
            ...
            <ul className="space-y-2">
              {history.map((item) => (
                <li key={item.selection_id} className="rounded border px-3 py-2 text-sm">
