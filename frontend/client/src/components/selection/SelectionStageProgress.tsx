...
const STAGES_TWO = [
  { step: 1, label: "Выбор из карточек" },
  { step: 2, label: "Ввод параметров" },
] as const;

const STAGES_THREE = [
  { step: 1, label: "Выбор из карточек" },
  { step: 2, label: "Линейка гидромодуля" },
  { step: 3, label: "Ввод параметров" },
] as const;

export type SelectionStageProgressProps =
  | { variant?: "two"; current: 1 | 2 }
  | { variant: "three"; current: 1 | 2 | 3 };

export function SelectionStageProgress(props: SelectionStageProgressProps) {
  const variant = props.variant ?? "two";
  const stages = variant === "three" ? STAGES_THREE : STAGES_TWO;
