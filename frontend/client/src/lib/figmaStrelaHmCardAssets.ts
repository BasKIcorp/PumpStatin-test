...
import bpsCHuynyaCardUrl from "@/assets/hm-cards/bps-c-huynya.png";

/** Встроенные в бандл картинки (не зависят от public на сервере и Figma). */
const BUNDLED_HM_CARDS: Partial<Record<HydromoduleLineId, string>> = {
  "bps-c-huynya": bpsCHuynyaCardUrl,
};
...
export function strelaHmCardImage(id: HydromoduleLineId, serverUrl?: string | null): string {
  ...
  const bundled = BUNDLED_HM_CARDS[id];
